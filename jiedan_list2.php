...
<form id="form1" name="form1" method="post" action="">
  搜索: 订单号：<input name="dingdanhao" type="text" id="dingdanhao" style='border:solid 1px #000000; color:#666666;width:80px' /> 联系人：<input name="lianxiren" type="text" id="lianxiren" style='border:solid 1px #000000; color:#666666;width:80px' /> 联系人手机：<input name="lianxirenshouji" type="text" id="lianxirenshouji" style='border:solid 1px #000000; color:#666666;width:80px' /> 状态：<select name='zhuangtai' id='zhuangtai' style='border:solid 1px #000000; color:#666666;'><option value="">所有</option><option value="已接单">已接单</option><option value="工作中">工作中</option><option value="已结算">已结算</option><option value="申请重新发起订单">申请重新发起订单</option><option value="申请取消订单">申请取消订单</option></select> 排序
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td width="150" align="center" bgcolor="#CCFFFF" >操作</td>
  </tr>
  <?php 
  

  
    $sql="select * from jiedan where jiedanren='".$_SESSION['username']."'";
	...
	for($i=$pagecurrent*$pagelarge-$pagelarge;$i<$ddddd;$i++)
{
  ...
  <tr>
	...
	<!--被申请取消的订单-->
	<?php if(mysql_result($query,$i,zhuangtai)=="申请取消订单"){?>
	<a href="jiedan_ed_chexiao.php?dingdanhao=<?php echo mysql_result($query,$i,"dingdanhao");?>&tablename=jiedan" onclick="return confirm('确认同意？')">同意取消订单</a><br>
	<a href="del.php?dingdanhao=<?php echo mysql_result($query,$i,"dingdanhao");?>&tablename=jiedan" onclick="return confirm('确认拒绝申请？')">拒绝取消申请</a><br>
	<a href="jiedan_detail.php?id=<?php echo mysql_result($query,$i,"id");?>">详细</a></td>
	<?php }?>
	<!--已结算订单-->
	...
	<a href="jiedan_ed_chexiao.php?dingdanhao=<?php echo mysql_result($query,$i,"dingdanhao");?>&tablename=jiedan" onclick="return confirm('确认要放弃订单？')">申请撤销订单</a><br>
	<a href="jiedan_end.php?dingdanhao=<?php echo mysql_result($query,$i,"dingdanhao");?>&tablename=jiedan" onclick="return confirm('确认订单已结算？')">订单已结算</a><br>
	<a href="jiedan_detail.php?id=<?php echo mysql_result($query,$i,"id");?>">详细</a></td>
	<?php }?>
	 <!--提交撤销申请的订单-->
	<?php if(mysql_result($query,$i,zhuangtai)=="申请撤销订单"){?>
	<a href="jiedan_ed_chexiao.php?dingdanhao=<?php echo mysql_result($query,$i,"dingdanhao");?>&tablename=jiedan" onclick="return confirm('确认要取消撤销申请？')">取消撤销订单申请</a><br>
	<a href="jiedan_detail.php?id=<?php echo mysql_result($query,$i,"id");?>">详细</a></td>
	<?php }?>
...
<p align="center"><a href="jiedan_list2.php?pagecurrent=1">首页</a>, <a href="jiedan_list2.php?pagecurrent=<?php echo $pagecurrent-1;?>">前一页</a> ,<a href="jiedan_list2.php?pagecurrent=<?php echo $pagecurrent+1;?>">后一页</a>, <a href="jiedan_list2.php?pagecurrent=<?php echo $pagecount;?>">末页</a>, 当前第<?php echo $pagecurrent;?>页,共<?php echo $pagecount;?>页 </p>
